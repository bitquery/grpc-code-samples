...
class CoreCastClient:
    ...
    def connect(self) -> None:
        ...
        if (
            not self.config.server.authorization
            or self.config.server.authorization == "ory_at_"
        ):
            logger.error("=" * 80)
            ...
            logger.error("")
            logger.error(
                "You have not set a valid authorization token in config.yaml"
            )
            logger.error("")
            logger.error(
                "Current value: authorization: \"%s\"",
                self.config.server.authorization or "(empty)",
            )
            logger.error("")
            logger.error("To fix this:")
            logger.error(
                "  1. Get your auth token from: https://docs.bitquery.io/docs/authorisation/how-to-generate/"
            )
            logger.error("  2. Open config.yaml")
            logger.error(
                "  3. Replace 'ory_at_' with your actual token (starts with 'ory_at_')"
            )
            logger.error("")
        ...
        logger.debug(
            f"Connecting to gRPC server: {self.config.server.address}"
        )

        # Create channel
        self.channel = grpc.secure_channel(
            self.config.server.address, credentials, options=options
        )
    ...
    def _create_metadata(self) -> List[tuple]:
        ...
        if self.config.server.authorization:
            metadata.append(
                ("authorization", f"Bearer {self.config.server.authorization}")
            )
            logger.debug("Authorization metadata attached")
        else:
            logger.warning(
                "No authorization token provided - connection may fail"
            )
        return metadata

    def _addr_filter_from_slice(
        self, addresses: List[str]
    ) -> Optional[request_pb2.AddressFilter]:
    ...
    def stream_dex_trades(self):
        ...
        req = request_pb2.SubscribeTradesRequest(
            program=self._addr_filter_from_slice(
                self.config.filters.programs
            ),
    ...
    def stream_transfers(self):
        ...
        req = request_pb2.SubscribeTransfersRequest(
            ...
            receiver=self._addr_filter_from_slice(
                self.config.filters.receivers
            ),
            token=self._addr_filter_from_slice(self.config.filters.tokens),
        )
    ...
    def stream_balances(self):
        ...
        req = request_pb2.SubscribeBalanceUpdateRequest(
            address=self._addr_filter_from_slice(
                self.config.filters.addresses
            ),
            token=self._addr_filter_from_slice(self.config.filters.tokens),
        )
        ...
        try:
            for msg in stream:
                try:
                    ...
                    print(
                        f"Received Timestamp: {received_timestamp.isoformat()}"
                    )
                    print(f"{'=' * 80}\n")
